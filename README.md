...
- GitHub Actions：测试、15 分钟监测、失败通知、GitHub Pages 部署。
...
- [Alpha Vantage 官方文档](https://www.alphavantage.co/documentation/) 区分历史、15 分钟延迟和实时权限；免费额度和端点可能调整。
...
- `monitor.yml`：工作日每 15 分钟被 GitHub 调度；只在配置真实 Provider 后启用定时任务，优先在对应交易时段请求，避免浪费免费额度；支持手动强制运行。
...
- `data/state.json`：本地去重状态，已被 `.gitignore` 排除。
...
**为什么 15 分钟没有准时运行**：GitHub Actions cron 不是实时调度器，高峰期可能延迟。系统已如实显示这个限制。
