...
- AKShare 免费 ETF 15分钟行情执行实验性盘中风险观察；当前5只ETF每轮约发出5次请求。
...
- GitHub Actions：测试、交易时段盘中风险观察、收盘分析、失败通知、GitHub Pages 部署。
...
```powershell
python -m venv .venv
.\.venv\Scripts\python -m pip install -e ".[dev,yfinance,intraday]"
...
正式日线流程只调用服务商提供的接口。盘中辅助流程通过 AKShare 聚合公开网页行情，已明确标记为实验性来源；可能因来源条款、访问限制或页面结构变化而中断，不应用于必须执行的止损。
...
| `akshare` | 否 | ETF 15分钟行情，仅用于盘中风险观察 | 实验性免费来源，可能延迟或中断 |
...
- `intraday.yml`：工作日交易时段每15分钟调度，程序严格过滤午休和非交易时间；通过 AKShare 逐只检查5只ETF。
...
- `data/intraday.json`：实验性盘中快照、来源状态和本轮风险统计。
...
**盘中提醒能否代替券商止损**：不能。免费网页来源和 GitHub Actions 都可能延迟或中断；必须执行的止损应使用券商条件单。
